...
**Compression vs. downstream.** That intrinsic compression does not monotonically predict downstream quality is established for *text* by PathPiece (Schmidt et al., EMNLP 2024) and by Lotz et al. (ACL 2025), and the fixed-model tokenizer-swap protocol originates in Ali et al. (Findings of NAACL 2024). In the *raster-image* domain, the relationship is shown to be capacity-dependent for discrete autoregressive generation (arXiv:2412.16326, NeurIPS 2025) — more-compressed stage-1 latents can *help* smaller stage-2 models even as reconstruction worsens. We contribute the first *vector-graphics* instance, isolated by a controlled tokenizer-swap and grounded in render fidelity — a setting where, in contrast to that raster-image result, more compression never reliably helps and on icons monotonically hurts.
...
Beyond bare commands and coordinates, L1 inserts two families of *auxiliary geometric markers*: per-anchor **continuity** flags (smooth/cusp/…) and quantized **curvature** bins on curve segments. These are not free — on the held-out svg-icons split they are **24.2% of all L1 tokens** (continuity 18.6%, curvature 5.6%; mean 120.8 tok/icon, of which 29.2 are markers). Because they inflate sequence length, their inclusion is a deliberate design choice that an ablation must justify rather than assume. It does not: §5.1 shows the markers are derivable (zero reconstruction effect) yet *raise* held-out NLL by ~12%, so we recommend a **lean L1** that omits them. We retain them in the as-shipped default used by §5.2–5.4, which makes those results a conservative reading of the geometric advantage; since v1.1 the released tokenizer exposes lean L1 as a first-class option (`lean=True`), verified byte-identical in reconstruction and grammar-valid on the full 2,726-icon validation corpus.
...
GeomTok-L1's coordinate fidelity is **1.78px mean / 3.37px max** on a 300px canvas (within the analytic bound), with render SSIM **0.929** — the fidelity cost of the 3.54× compression (the round-trip is geometric and bounded-error, not lossless). The error *tail* backs the bounded-error claim: on a separate 2,726-icon validation corpus (Tabler/Feather/Heroicons/Lucide; 92,840 coordinates), p50/p90/p99 = 2.36/3.25/3.32px — the tail is pinned at the quantization ceiling rather than spreading — while **61.7% of coordinates exceed a 2px perceptibility threshold** (the honest per-coordinate cost of the 64×64 grid), and only 2/92,840 coordinates exceed the codec's operational error ceiling (3.4px), both caused by out-of-viewBox clamping and both flagged by the tokenizer's own `clamped` warning. Hand-crafted shape tokens (L2) fire on **0%** of real icons and add nothing.

**Auxiliary-marker ablation (the markers do not earn their cost — a negative result).** L1 also emits per-anchor *continuity* and per-segment *curvature* markers (§3.3), 24.2% of all L1 tokens. We test directly whether they help. First, they are **derivable, not load-bearing**: the detokenizer parses but never uses them (reconstruction reads only command + coordinate tokens), and stripping them is **byte-identical on 57/57 held-out icons** — zero fidelity cost. Second, on the tokenizer-swap protocol (980 train / 205 test, 3 seeds, 25 epochs) stripping them *improves* held-out NLL from **687.2 ± 2.0 to 602.4 ± 2.7 bits/icon** (−84.8, ≈12%, far beyond seed noise) *and* shortens sequences (120.8 → 91.6 tokens/icon). So a model spends ~12% of its modeling budget predicting tokens it can already derive and that the decoder ignores. This is the same mechanism as §5.3 read in reverse: removing *zero-information* tokens compresses *and* helps, whereas BPE merges compress by raising per-token entropy and hurt — it is **what** you compress, not how much. We therefore report a **lean L1** (no markers) as the recommended substrate; the §5.2–5.4 results use the as-shipped full L1, making the L1-vs-text-BPE gap a *conservative* lower bound on the geometric advantage.
...
| Budget | char-BPE | L1 unconstrained | L1 boundary-constrained | L1 + HiVG-style merge rule |
|---|---|---|---|---|
| 0 | 4893 | 264 | 264 | 264 |
| 1000 | 824 | **172** | 222 | 236 |

encoding bits/icon @1000: char 7245 ≫ L1 1687 < boundary 1890 < HiVG-style 1934 — identical ranking. **At matched budget, unconstrained learned merges over primitive tokens use 27% fewer tokens than the HiVG-style structure-constrained merge rule (172 vs 236)**: the constraint forbids cross-command repeated patterns that unconstrained BPE exploits. (We compare the *merge rule* — HiVG's command-rooted, geometry-constrained pairing applied to our L1 stream — not HiVG's full representation, which also uses relative-offset coordinate tokens; this isolates the constraint's effect on a shared substrate.) (L1 encoding-bits/icon saturate by ~100 merges; we report bits alongside tokens to avoid overstating merge gains.) This is an intrinsic-compression comparison at fixed budget, not a claim of downstream superiority for L1+BPE — see §5.3.
...
> **Absolute NLL is comparable only within a panel.** This main table (d=192, 25 epochs, 1,045/213 split) and the budget-sweep, capacity, and second-corpus panels below use different training lengths, splits, and corpora (see REPRO.md §2–3), so absolute bits/icon shift between panels (e.g. L1 = 576 here vs 608 at the budget-sweep's N=0). All cross-panel claims rest only on the *sign and direction of the L1-vs-baseline gap within each panel*, never on comparing absolute values across panels. The gap here is overwhelming (Welch t ≈ −48 for char-BPE vs L1 over 3 seeds).
...
**Capacity trend (the substrate gap does not vanish with scale).** Running the L1-vs-L1+BPE(1k) swap at four model sizes from 0.9M to 9.3M parameters, L1 wins at every size and the gap *widens and then plateaus* (35→47→62→62), rather than shrinking toward zero as a small-model artifact would:
...
The L1>L1+BPE advantage is not a small-model artifact across this 10× parameter range; the gap stops widening by 4.6M (62, unchanged at 9.3M), so we claim persistence, not unbounded growth, and whether it holds at 100M–1B is future work (§10). (Single seed per size; treated as a trend, not a precision estimate.)
...
In this small-model regime, a text tokenizer yields **0% renderable SVG** under the same sampling that gives GeomTok 84% (Figure 4) — geometric tokens appear *required* for renderable output here, not merely more efficient (we scope this to the 2.5M-parameter setting; large VLMs with massive SVG corpora may close the gap). L1 yields **2.4× more renderable samples than L1+BPE (84% vs 35%)**: aggressive merges that win on compression halve generation validity. On renderable samples FID is comparable; no mode collapse (diversity ≈0.9), no memorization (novelty ≈0.84). **FID-lite, diversity, and novelty are conditional on rendering** and so are computed over different per-arm subsets (L1 ≈168/200 renderable, L1+BPE ≈70/200; char-BPE 0, hence "—"); they compare *only the samples each arm got right*, not the arms head-to-head, and a survivorship bias favors the lower-yield arm. The headline of §5.4 is the renderable rate (84% vs 35% vs 0%), where there is no such confound; FID/diversity/novelty are reported as sanity checks that the renderable L1 samples are neither collapsed nor memorized.
...
2. ✅ **Capacity trend** (§5.3) — the substrate gap holds (widens then plateaus, 35→62 bits) over 0.9M–9.3M params.
...
5. ✅ **Auxiliary-marker ablation** (§5.1) — curvature+continuity tokens (24.2% of L1) are derivable (57/57 byte-identical round-trip) yet raise held-out NLL by 84.8 bits (~12%); lean L1 recommended.
6. ✅ **Coordinate-error tail** (§5.1) — measured on the bundled 2,726-icon validation corpus: p99 = 3.32px (pinned at the quantization ceiling), >2px perceptible rate 61.7%, beyond-ceiling tail 2/92,840 (both clamp-attributed). `.research/results_e6_error_tail.txt`.
7. **Nice-to-have:** StrokeNUWA VQ point on the compression–fidelity scatter; small human forced-choice eval; Figures 1, 3, 4 (Figure 2 done).
