...
이론적 근거 (일부 v0.7에서 철회 — 아래 주의):
    1. 수렴 가속: ⚠ **철회됨**. 통제 실험(.research/exp_init_norm_confound.py)에서
       기하 구조 없는 random_unitnorm(무작위 벡터를 단위 노름으로만 재조정)이
       동일/더 나은 val_loss를 보였다 (random 4.51 → random_unitnorm 3.40 →
       HMN 3.47). 관측된 "수렴 가속"은 weight tying 하 임베딩 노름 효과이지
       기하 구조 효과가 아니다. HMN은 노름 정합 이상의 학습 이득을 주지 않는다.
    2. 기하학적 귀납 편향: 공간적 근접성이 임베딩 근접성으로 (사실 — 0.52
       코사인은 재현되나, 위 1번에 따라 다운스트림 이득은 미입증)
